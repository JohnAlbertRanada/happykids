...
export default function AdminActivity() {
  ...
  const fetchData = async () => {
    ...
    const userRef = collection(db, "word_pronunciation");
  ...
  async function handleDelete() {
    try {
      const docRef = doc(db, "word_pronunciation", selectedItemId);
  ...
  async function handleAdd() {
    ...
    if (words.length < addedWord.level) {
      console.log("CALLED");
      try {
        const docRef = await addDoc(collection(db, "word_pronunciation"), {
          word: addedWord.word,
  ...
  const AddItem = () => {
    return (
      <div className="rounded p-5 flex flex-col bg-white">
        <p className="text-2xl font-medium text-[#766A6A] mb-5">Add Word</p>
        <InputField
          label="Word"
          name="word"
          type="text"
  ...
  const EditItem = () => {
    return (
      <div className="rounded p-5 flex flex-col bg-white">
        <p className="text-2xl font-medium text-[#766A6A] mb-5">Edit Word</p>
        <InputField
          label="Word"
          name="word"
          type="text"
        ...
        <>
          ...
          <div className="flex flex-row ml-auto items-center rounded bg-[#d9d9d9] p-1 mx-10 mb-5 gap-2 w-min">
            ...
            <button
              ...
              onClick={() =>
                router.push("/admn/activity/sentence_pronunciation")
              }
            >
            ...
            <button
              className="text-black rounded w-48 h-10"
              onClick={() => router.push("/admn/activity/scripted_speech")}
            >
              Scripted Speech
            </button>
          </div>
